...
GoogleNet = Sequential([
    # Initial Convolution
    (Conv2D(64, (7, 7), strides=(2, 2), padding='same', activation='relu',  input_shape=(IMAGE_SIZE[0], IMAGE_SIZE[1], 1))),
    ...
    (Conv2D(96, (5, 5), padding='same', activation='relu')),
    (MaxPooling2D((3, 3), strides=(1, 1), padding='same')),
...
if (os.path.exists('GoogleNet.tfl')):
    GoogleNet=joblib.load('GoogleNet.tfl')
else:
    # reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.2, patience=5, min_lr=0.0001)
    ThisModel = GoogleNet.fit(X_train, Y_train, batch_size=64, validation_data=(X_test, Y_test),
                                epochs=5,callbacks=[reduce_lr], verbose=1)
    joblib.dump(GoogleNet, "GoogleNet.tfl")
...
with open("47.csv", "w", newline="") as f:
    ...
    for image in tqdm(os.listdir('C:/Users/monasser/Desktop/nural_project/dataset/test')):
        ...
        prediction = GoogleNet.predict(img_test)[0]
