...
def VisionTransformer(image_size, patch_size, num_classes, num_blocks, embed_dim, num_heads, ff_dim, rate=0.1):
    ...
    # Patching image into patches
    patches = layers.Patching2D(patch_size)(inputs)
    patches = layers.Reshape((patches.shape[1] * patches.shape[2], patches.shape[-1]))(patches)

    # Projection and Positional Embeddings
    embeddings = layers.Embedding(input_dim=patches.shape[1], output_dim=embed_dim)(patches)
...
flattened_output = layers.Flatten()(cnn_model)

# Reshape the output of the CNN to (num_patches, embed_dim)
reshape_output = layers.Reshape((num_patches, embed_dim))(cnn_model)
...
combined_features = layers.Concatenate()([flattened_output, positional_embeddings])
...
#
# ThisModel = combined_model.fit(X_train, Y_train, batch_size=64, validation_data=(X_test, Y_test),
#                             epochs=5, callbacks=[reduce_lr], verbose=1)
# joblib.dump(combined_model, "combined_model.tfl")
#
